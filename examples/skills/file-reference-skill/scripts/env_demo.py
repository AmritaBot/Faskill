...
This script demonstrates how faskill automatically injects environment
...
Usage:
    This script is designed to be executed by faskill's script executor.
...
def main():
    ...
    # Read arguments from stdin (standard faskill pattern)
    try:
    ...
    faskill_version = os.environ.get("FASKILL_VERSION", "unknown")
    ...
    print(f"  faskill_VERSION  = {faskill_version}")
    print()
    print("Arguments Received:")
    print(f"  {json.dumps(args, indent=2)}")
    print()
    print("Example Use Cases:")
    print("  1. Locate skill files:")
    print("     config_path = Path(os.environ['SKILL_BASE_DIR']) / 'config.json'")
    print(f"     → {Path(skill_base) / 'config.json'}")
    print()
    print("  2. Contextual logging:")
    print("     logger.info(f'[{os.environ[\"SKILL_NAME\"]}] Processing...')")
    print(f"     → [{skill_name}] Processing...")
    print()
    print("  3. Version-specific behavior:")
    print("     if os.environ['SKILL_VERSION'] >= '2.0.0':")
    print("         use_new_api()")
    print()
    print("=" * 60)
